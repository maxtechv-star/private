...
`);
    ...
    if (global.db.settings && global.db.settings.areact2 && isMessage && isCmd2) {
      try {
        ...
        console.error("⚠️ An error occurred while processing AutoReact:", error.message || error);
      }
    }   
...
if (global.db.users[m.sender].banned) return m.reply(msg.ban)
  ...
  try {
    ...
    for (let video of result) {
      await VeronicaX.sendFile(m.chat, video.url, 'instagram.mp4', '© 𝓢𝓾𝓬𝓮𝓼𝓼 𝓫𝔂 𝓟𝓲𝔁𝓮𝓵 𝔀𝓪 𝓫𝓸𝓽', m)
    }
  } catch (err) {
    console.error(err)
...
await VeronicaX.sendMessage(m.chat, {image: {url:anuan}, caption: `𝓢𝓾𝓬𝓮𝓼𝓼 𝓫𝔂 𝓟𝓲𝔁𝓮𝓵 𝔀𝓪 𝓫𝓸𝓽`, fileLength: "999", viewOnce : true},{quoted: m })
} else if (/video/.test(mime)) {
let anuanuan = await VeronicaX.downloadAndSaveMediaMessage(quoted)
await VeronicaX.sendMessage(m.chat, {video: {url:anuanuan}, caption: `𝓢𝓾𝓬𝓮𝓼𝓼 𝓫𝔂 𝓟𝓲𝔁𝓮𝓵 𝔀𝓪 𝓫𝓸𝓽`, fileLength: "999", viewOnce : true},{quoted: m })
}
}
break
...
VeronicaX.sendMessage(m.chat, { image: { url: haldwhd }, caption: `𝓢𝓾𝓬𝓮𝓼𝓼 𝓫𝔂 𝓟𝓲𝔁𝓮𝓵 𝔀𝓪 𝓫𝓸𝓽` }, { quoted: m })
}
break
...
case 'joke':
  ...
  "Why did the music teacher go to jail? Because she got caught with the wrong notes."
    ...
    await VeronicaX.sendMessage(m.chat, { text: `😂 *Here’s a joke for you:*\n\n${getJoke}` }, { quoted: m })
    break
   //====================================================
   case 'tonguetwister': {
  if (m.isGroup && global.db.groups[m.chat].banned) return;
...
break
  ...
  "Never have I ever waved at someone who wasn’t waving at me."
    ...
    await VeronicaX.sendMessage(m.chat, { text: `🙅‍♂️ *Never Have I Ever...*\n\n${never}` }, { quoted: m })
    break
  ...
  "Stay In 🛏️ or Go Out 🎉?\nReact 🔥 for Stay In or 💧 for Go Out"
    ...
    await VeronicaX.sendMessage(m.chat, { text: `⚖️ *This or That?*\n\n${tOrT}` }, { quoted: m })
    break
  ...
  "Would you rather live in a video game 🎮 or a movie 🎬?\n_React your answer with the emoji on the question!_"
    ...
    await VeronicaX.sendMessage(m.chat, { text: `🤔 *Would You Rather...*\n\n${wyr}` }, { quoted: m })
    break
